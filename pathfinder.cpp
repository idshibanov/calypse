...
AStarSearch::AStarSearch(shared_ptr<GameMap> map) {
    _map = map;
}
...
std::vector<AStarNode> AStarSearch::searchPath(size_t startX, size_t startY, size_t goalX, size_t goalY) {
    ...
    clearData();
    ALLEGRO_COLOR closedColor = al_map_rgb(20, 50, 180);
    ALLEGRO_COLOR visitedColor = al_map_rgb(100, 150, 250);
    ALLEGRO_COLOR openColor = al_map_rgb(200, 50, 140);
    ...
    while(!_openSet.empty()) {
        ...
        for (auto itVisit = _visitedSet.begin(); itVisit != _visitedSet.end(); ++itVisit) {
            int _xpos = (*itVisit)->_mapX * TD_TILESIZE_X;
            int _ypos = (*itVisit)->_mapY * TD_TILESIZE_Y;
            if((*itVisit)->_closed) {
                al_draw_filled_rectangle(_xpos, _ypos, _xpos + TD_TILESIZE_X, _ypos+TD_TILESIZE_Y, closedColor);
            } else {
                al_draw_filled_rectangle(_xpos, _ypos, _xpos + TD_TILESIZE_X, _ypos+TD_TILESIZE_Y, visitedColor);
            }
        }
		al_flip_display();
